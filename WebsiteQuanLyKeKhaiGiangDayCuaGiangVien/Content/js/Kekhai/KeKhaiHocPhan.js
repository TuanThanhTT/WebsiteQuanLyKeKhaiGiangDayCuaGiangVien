...
function updatePhanTrangTableChoDuyet(totalPages, currentPage) {
    var pagination = document.getElementById("phanTrangTableChoDuyet");
    pagination.innerHTML = ''; // Xóa nội dung phân trang cũ

    // Nút Previous
    var prev = document.createElement('li');
    prev.className = "page-item" + (currentPage === 1 ? " disabled" : "");
    prev.innerHTML = '<a class="page-link" href="#" onclick="loadTablePhanCongChoDuyet(' + (currentPage - 1) + ')">Trước</a>';
    pagination.appendChild(prev);
    ...
    for (let i = 1; i <= totalPages; i++) {
        var page = document.createElement('li');
        page.className = "page-item" + (i === currentPage ? " active" : ""); // Đặt lớp active cho trang hiện tại
        page.innerHTML = '<a class="page-link" href="#" onclick="loadTablePhanCongChoDuyet(' + i + ')">' + i + '</a>';
        pagination.appendChild(page);
    }
    ...
    var next = document.createElement('li');
    next.className = "page-item" + (currentPage === totalPages ? " disabled" : "");
    next.innerHTML = '<a class="page-link" href="#" onclick="loadTablePhanCongChoDuyet(' + (currentPage + 1) + ')">Sau</a>';
    pagination.appendChild(next);
}
